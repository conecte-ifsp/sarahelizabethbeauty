const seta = document.getElementById("seta-projetos");
const img = document.getElementById("img-projetos");
...
let mostrandoPrimeiro = true; // controla qual imagem tá aparecendo

seta.addEventListener("click", () => {
    // adiciona a animação de saída
    img.classList.add("slide-out");
    texto.classList.add("slide-out");

    setTimeout(() => {
        if (mostrandoPrimeiro) {
            // troca pra segunda
            img.src = "assets/img/2mulhersessao3.jpg";
            titulo.textContent = "CABELEIREIRA";
            expert.textContent = "PENTEADOS";
            peles.textContent = "e tranças";
            descricao.textContent = "com acabamento impecável e proteção para seus fios";
        } else {
            // volta pra primeira
            img.src = "assets/img/mulhersessao3.jpg";
            titulo.textContent = "Makeup";
            expert.textContent = "EXPERT EM";
            peles.textContent = "peles";
            descricao.textContent = "Negra, clara, mista e madura.";
        }

        // alterna o estado
        mostrandoPrimeiro = !mostrandoPrimeiro;

        // remove animação de saída e aplica entrada
        img.classList.remove("slide-out");
        texto.classList.remove("slide-out");
        ...
        setTimeout(() => {
            ...
            texto.classList.add("slide-in-active");
        }, 50);

        // limpa classes depois da animação
        setTimeout(() => {
            img.classList.remove("slide-in", "slide-in-active");
            texto.classList.remove("slide-in", "slide-in-active");
        }, 700);
    }, 600);
});
...
// Adiciona SVGs ao logoArea
logoArea.innerHTML = svgBranco + svgColorido;

const svgBrancoEl = logoArea.children[0];
const svgColoridoEl = logoArea.children[1];

// Inicializa opacidade
svgBrancoEl.classList.add('logo-fade-in');
svgColoridoEl.classList.add('logo-fade-out');

const offset = 120; // troca antes da sessão

function trocarLogo() {
    const rect = sessaoSara.getBoundingClientRect();

    if (rect.top <= offset && rect.bottom > 0) {
        // entra na seção branca → mostrar colorido
        svgBrancoEl.classList.replace('logo-fade-in', 'logo-fade-out');
        svgColoridoEl.classList.replace('logo-fade-out', 'logo-fade-in');
    } else {
        // fora da seção → mostrar branco
        svgBrancoEl.classList.replace('logo-fade-out', 'logo-fade-in');
        svgColoridoEl.classList.replace('logo-fade-in', 'logo-fade-out');
    }
}
